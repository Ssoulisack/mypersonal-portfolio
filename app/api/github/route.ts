...
const username: string = GITHUB_CONFIG.GITHUB_USERNAME || "your-github-username";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (useCache) {
      ...
      if (cached && Date.now() - cached.timestamp < GITHUB_CONFIG.CACHE_DURATION) {
    ...
    try {
      const response = await githubAxios.post<GitHubResponse>('', BODY_QUERY.GET_CONTRIBUTIONS({ username }));
      data = response.data;
